...
//            String columnDataStr = JSONObject.toJSONString(columnData);
            ...
            // 如果 updateInfo（列族） execute_time（列） 不存在值就插入数据，如果存在则返回false
            ...
            if (!res1) {
                // 如果 updateInfo（列族） execute_time（列） 不等于 val2，则插入put
...
//            boolean res3 = table.checkAndMutate(Bytes.toBytes(hbaseData.get("id")), Bytes.toBytes("updateInfo"))
...
//
//            if (!res3) {
//                // 如果 updateInfo（列族） exe_position（列） 不等于 val2，则插入put
...
//            put4.addColumn(Bytes.toBytes("updateInfo"), Bytes.toBytes("columnData"), Bytes.toBytes(columnDataStr));
//
//            boolean res5 = table.checkAndMutate(Bytes.toBytes(hbaseData.get("id")), Bytes.toBytes("updateInfo"))
...
//
//            if (!res5) {
//                boolean res6 = table.checkAndMutate(Bytes.toBytes(hbaseData.get("id")), Bytes.toBytes("updateInfo"))
//                        .qualifier(Bytes.toBytes("columnData"))
//                        .ifMatches(CompareOperator.NOT_EQUAL,Bytes.toBytes("val2"))
//                        .thenPut(put4);
//            }
...
//                put2.addColumn(Bytes.toBytes("updateInfo"), Bytes.toBytes("execute_time"), Bytes.toBytes(hbaseData.get("execute_time")));
                // 备注：这里要先从hbase表里取出当前列的值，然后再做判断
//                updateResult = table.checkAndMutate(Bytes.toBytes(hbaseData.get("id")), Bytes.toBytes("updateInfo"))
...
//                    Put put3 = new Put(Bytes.toBytes(hbaseData.get("id")));
//                    put3.addColumn(Bytes.toBytes("updateInfo"), Bytes.toBytes("exe_position"), Bytes.toBytes(hbaseData.get("logfileOffset")));
//                    boolean res3 = table.checkAndMutate(Bytes.toBytes(hbaseData.get("id")), Bytes.toBytes("updateInfo"))
//                            .qualifier(Bytes.toBytes("exe_position"))
//                            .ifNotExists()
//                            .thenPut(put3);
...
//                    if (!res3) {
//                        Put put4 = new Put(Bytes.toBytes(hbaseData.get("id")));
//                        put4.addColumn(Bytes.toBytes("updateInfo"), Bytes.toBytes("exe_position"), Bytes.toBytes(hbaseData.get("logfileOffset")));
//
//                        // 备注：这里要先从hbase表里取出当前列的值，然后再做判断
//                        // 如果row1 updateInfo exe_position 666 存在就插入新数据
